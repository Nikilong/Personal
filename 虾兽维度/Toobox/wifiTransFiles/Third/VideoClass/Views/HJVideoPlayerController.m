...
@property (nonatomic ,strong) HJVideoMaskView * maskView;  // 播放按钮栏

@property (nonatomic ,strong) HJVideoTopView * topView;  // 顶部工具栏

@property (nonatomic ,strong) HJVideoBottomView * bottomView;  // 底部进度条等

@property (weak, nonatomic)  UIScrollView *playlistV;
...
- (void)setupUI
{
    ...
    CGFloat btnH = 35;
    CGFloat btnW = 150;
    CGFloat playListVH = 180;
    UIScrollView *playListV = [[UIScrollView alloc] init];
    playListV.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
    self.playlistV = playListV;
    for (NSUInteger i = 0; i < self.videoList.count; i++) {
        XMWifiTransModel *model = self.videoList[i];
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, i * btnH , btnW, btnH)];
        btn.tag = i;
        [btn addTarget:self action:@selector(playlistButtonDidClick:) forControlEvents:UIControlEventTouchUpInside];
        [btn setTitle:model.pureFileName forState:UIControlStateNormal];
...
- (void)playlistButtonDidClick:(UIButton *)btn{
    XMWifiTransModel *model = self.videoList[btn.tag];
    self.videoIndex = btn.tag;
    [self setUrl:model.fullPath];
    self.playlistV.hidden = YES;
}
...
- (void)play{
    // 设置标题
    XMWifiTransModel *model = self.videoList[self.videoIndex];
    self.videoTitle = model.pureFileName;
    // 隐藏控制界面
    [self hideSomeViews];
    //若有记录则从记录播放
...
- (HJVideoTopView *)topView{
    if (!_topView) {
        ...
        _topView.showListBlock = ^(BOOL show){
            weakSelf.playlistV.hidden =  !weakSelf.playlistV.hidden;
            
            // 调整播放列表的x
            CGRect targetF = weakSelf.playlistV.frame;
            targetF.origin.x = kVideoScreenW - weakSelf.playlistV.frame.size.width - 10;
            weakSelf.playlistV.frame = targetF;
        };
    }
    return _topView;
}
...
- (HJVideoBottomView *)bottomView{
    if (!_bottomView) {
        ...
        _bottomView.nextVideoBlock = ^(){
            // 播放下一个视频
            weakSelf.videoIndex++;
            if (weakSelf.videoIndex > weakSelf.videoList.count - 1){
                weakSelf.videoIndex = 0;
            }
            XMWifiTransModel *model = weakSelf.videoList[weakSelf.videoIndex];
            [weakSelf setUrl:model.fullPath];
        };
        _bottomView.preVideoBlock = ^(){
            // 播放上一个视频
            if (weakSelf.videoIndex == 0 ){
                weakSelf.videoIndex = weakSelf.videoList.count - 1;
            }else{
                weakSelf.videoIndex--;
            }
            XMWifiTransModel *model = weakSelf.videoList[weakSelf.videoIndex];
...
- (void)setVideoList:(NSArray *)videoList{
    ...
    if(videoList.count > 1){
        ...
        self.playEndBlock = ^(){
            ...
            XMWifiTransModel *model = videoList[weakSelf.videoIndex];
            [weakSelf setUrl:model.fullPath];
        };
    }
