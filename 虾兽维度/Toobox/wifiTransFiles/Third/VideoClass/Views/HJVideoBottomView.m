...
static const CGFloat kNextpreBtnWidth = 44.f;
...
/** 上一个/下一个视频按钮 */
@property (nonatomic, strong) UIView *nextpreBtnContentV;
/** 播放时长Label */
...
- (void)setupUI
{
    ...
    [self addSubview:self.nextpreBtnContentV];
...
- (void)refreshUI{
    ...
    if (self.configModel && self.configModel.onlyFullScreen && !self.onlySlider){
        ...
        self.nextpreBtnContentV.frame = CGRectMake(0, 0, 2 *kNextpreBtnWidth, kNextpreBtnWidth);
...
- (void)adjustSubviesOnlyFull:(BOOL)onlyFull{
    ...
    CGFloat width = CGRectGetWidth(self.frame);
    CGFloat height = CGRectGetHeight(self.frame);
    CGFloat nextpreBtnTotalW = kNextpreBtnWidth * 2;
    self.playTimeLbl.frame = CGRectMake(nextpreBtnTotalW, 0, kTimeLabelWidth, height);
    
    self.totalDurationLbl.frame = CGRectMake(width - (onlyFull?0:height) - kTimeLabelWidth - nextpreBtnTotalW, 0, kTimeLabelWidth, height);
...
- (UIView *)nextpreBtnContentV{
    if(!_nextpreBtnContentV){
        _nextpreBtnContentV = [[UIView alloc] init];
        UIButton *nextVideoBtn = [HJViewFactory buttonWithNormalImage:[UIImage imageFromBundleWithName:@"btn_next"] selectedImage:nil];
        nextVideoBtn.frame = CGRectMake(kNextpreBtnWidth, 0, kNextpreBtnWidth, kNextpreBtnWidth);
        [_nextpreBtnContentV addSubview:nextVideoBtn];
        [nextVideoBtn addTarget:self action:@selector(nextVideo:) forControlEvents:UIControlEventTouchUpInside];
        UIButton *preVideoBtn = [HJViewFactory buttonWithNormalImage:[UIImage imageFromBundleWithName:@"btn_pre"] selectedImage:nil];
        [preVideoBtn addTarget:self action:@selector(preVideo:) forControlEvents:UIControlEventTouchUpInside];
        preVideoBtn.frame = CGRectMake(0, 0, kNextpreBtnWidth, kNextpreBtnWidth);
        [_nextpreBtnContentV addSubview:preVideoBtn];
    }
    return _nextpreBtnContentV;
}
...
- (HJBufferSlider *)bufferSlider
{
    if (!_bufferSlider) {
        ...
        [_bufferSlider setProgressTrackColor:[UIColor orangeColor]];
