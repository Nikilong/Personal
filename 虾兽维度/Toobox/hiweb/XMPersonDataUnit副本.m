...
+ (NSArray *)new_dealHomeDate:(NSString *)date{
    ...
    NSUInteger loca = 0;
    for (NSString *ele in strArr){
        
        if ([ele containsString:@"<span>"] && [ele containsString:@"<br>"]){
            loca = [ele rangeOfString:@"<span>"].location + 6;
//            NSLog(@"%zd--%@",strArr.count,[ele substringWithRange:NSMakeRange(loca,ele.length - loca - 4)]);

            [titleArr addObject:[ele substringWithRange:NSMakeRange(loca,ele.length - loca - 4)]];
        }
    }
    
    // 获取连接作品连接
    NSArray *urlArr = [self new_dealDateUrl:date logFlag:NO];
    
    // 获取封面(需要拼接)
    NSString *reguStrMsg = @"/[a-zA-Z0-9\\.\\-]*jpg";
    NSArray *imgArr = [self regularWithString:date regular:reguStrMsg logFlag:NO];
    
    NSMutableArray *resultArr = [NSMutableArray array];
    //https://www.javbus2.com/ABP-678,
//https://pics.javcdn.pw/thumb/6cp0.jpg
//    if (imgArr.count == urlArr.count && imgArr.count == titleArr.count){
    if ( urlArr.count  == titleArr.count * 2){
        for (NSUInteger i = 0; i < imgArr.count; i++) {
            XMSingleFilmModle *model = [[XMSingleFilmModle alloc] init];
//            model.imgUrl = [@"https://pics.javcdn.pw/thumb" stringByAppendingString:imgArr[i]];
            model.imgUrl = urlArr[i * 2 + 1];
            model.url = urlArr[i * 2];
...
+ (NSArray *)new_dealDatePicture:(NSString *)date{
    NSArray *urlArr = [self new_dealDateUrl:date logFlag:YES];
    NSMutableArray *resultArr = [NSMutableArray array];
    for (NSString *ele in urlArr){
        XMSingleFilmModle *model = [[XMSingleFilmModle alloc] init];
        model.imgUrl = ele;
        model.title = nil;
        model.url = nil;
...
+ (NSArray *)new_dealDateAcotr:(NSString *)date{
    ...
    for (NSString *ele in strArr){
        ...
        if ([ele containsString:@"<span>"] && [ele containsString:@"</span>"]){
            NSUInteger loca = [ele rangeOfString:@"<span>"].location + 6;
//            NSLog(@"%zd--%@",strArr.count,[ele substringWithRange:NSMakeRange(loca,ele.length - loca - 7)]);
            [nameArr addObject:[ele substringWithRange:NSMakeRange(loca,ele.length - loca - 7)]];
        }
    }
...
// 处理单个影片中的同类影片
+ (NSArray *)new_dealRelateFilmArr:(NSString *)date{
    NSArray *strArr = [date componentsSeparatedByString:@"\n"];
    // 获得演员名称(需要提取)
    NSMutableArray *nameArr = [NSMutableArray array];
    for (NSString *ele in strArr){
    ...
    // 获取演员个人主页连接
    NSArray *urlArr = [self new_dealDateUrl:date];
